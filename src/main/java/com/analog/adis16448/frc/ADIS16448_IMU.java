  ...
  private void acquire() {
    ...
    while (!m_freed.get()) {
      // Waiting for the buffer to fill...
  	  ...
  	  data_count = m_spi.readAutoReceivedData(ByteBuffer.wrap(buffer),0,0); // Read number of bytes currently stored in the buffer
  	  array_offset = data_count % 28; // Look for "extra" data
  	  data_count = data_count - array_offset; // Discard "extra" data
  	  m_spi.readAutoReceivedData(ByteBuffer.wrap(buffer),data_count,0); // Read data from DMA buffer
